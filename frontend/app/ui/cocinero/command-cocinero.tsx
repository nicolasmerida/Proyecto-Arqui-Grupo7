...
export default function CommandCocinero({command, state, lastUpdate} : CommandProps) {
    ...
    const cambiarEstado = async (item: Item_Pedido, nuevo: EstadoItem) => {
        // Enviar el ID del item completo como body y el estado como param
        await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL}/api/items-pedido/estado?nuevoEstado=${nuevo}`, {
            method: "PUT",
            headers: {
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 rounded-md gap-2">
                {
                    items.map((item) => (
                        <div key={`${item.id.numeroComanda}-${item.id.idPlato}`} className="flex flex-col items-center justify-between p-2 border rounded-md gap-2 bg-gray-50">
                            ...
                            <button 
                                className="w-full flex justify-center py-1 rounded-md border hover:bg-gray-200 transition-colors" 
                                onClick={() => cambiarEstado(item, EstadoItem.Preparacion)}
                                title="Avanzar estado"
                            >
                                {(state === EstadoComanda.Abierta) ? <HiOutlineArrowSmRight className="text-blue-500" /> : (
                                    (state === EstadoComanda.Preparacion) ? <HiOutlineCheck className="text-green-500" /> : (
                                    (state === EstadoComanda.Lista) ? <HiOutlineArrowSmRight className="text-gray-500" /> : <HiOutlineX className="text-red-500" />
                                    ))
                                }
                            </button>
                        </div>
